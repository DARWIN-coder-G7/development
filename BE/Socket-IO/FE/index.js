...
`;
        ...
        editButton.onclick = ()=>{updateItem(todoItem)};
        deleteButton.onclick = ()=>{deleteItem(todoItem.id)};
...
function addItem() {
    ...
    if (value) {
        socket.emit('newItem', value);
        todoItemInput.value = '';
    }
}

function updateItem(item) {
const promptBox = prompt("Update Todo",item.value);
if(promptBox){
socket.emit('updateItem',{id:item.id,value:promptBox});
}
}
